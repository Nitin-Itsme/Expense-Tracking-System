...
def fetch_expenses_for_date(expense_date):
    logger.info("Fetching expenses for date called with {expense_date}")
    with get_db_cursor() as cursor:
...
def insert_expense(expense_date, amount, category, notes):
    logger.info("insert_expense for date called with date:{expense_date} , amount:{amount} , category:{category} , notes:{notes}")
...
def delete_expenses_for_date(expense_date):
    logger.info("delete_expenses_for_date for date called with {expense_date}")
    with get_db_cursor(commit=True) as cursor:
        cursor.execute("DELETE FROM expenses WHERE expense_date = %s", (expense_date,))


def fetch_expense_summary(start_date, end_date):
    logger.info("fetch_expense_summary called with start: {start_date} end: {end_date}")
    with get_db_cursor() as cursor:
        cursor.execute(
